...
def rmse(img1, img2):
    ...
    diff = np.sqrt(np.sum((img1_ - img2_) ** 2) / (img1.shape[0] * img1.shape[1] * img1.shape[2]))
...
def _qindex(img1, img2, block_size=8):  # 1 denoting the best fidelity to reference.
...
def ref_evaluate(pred, gt, scale=4, dynamic_range=1):
    # reference metrics
    c_psnr = psnr(pred, gt, dynamic_range=dynamic_range)
    c_ssim = ssim(pred, gt, dynamic_range=dynamic_range)
    c_sam = sam(pred, gt)
    c_ergas = ergas(pred, gt, scale=scale)
    c_scc = scc(pred, gt)
    c_q = qindex(pred, gt)
    c_rmse = rmse(pred, gt)
...
def no_ref_evaluate(pred, pan, hs, scale=4):
    # no reference metrics
    #pred:[h,w,c][c,h,w]
    c_D_lambda = D_lambda(pred, hs)
    c_D_s = D_s(pred, hs, pan, scale=scale)
    c_qnr = qnr(pred, hs, pan, scale=scale)
